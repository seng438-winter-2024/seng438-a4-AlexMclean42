...
//    ------------------- Mutated test ------------------- 
    ...
    @Test
    public void testEqual_MutatedSubstitute1WithMinus1() {
        assertTrue(DataUtilities.equal(null, null)); // Mutated behavior: Substituted 1 with -1
    }
    @Test
    public void testEqual_MutatedSubstitute1WithMinus12() {
        double[][] a = null;
        double[][] b = {{-1.0}};
        assertFalse(DataUtilities.equal(a, b));
    }
    @Test
    public void testEqual_MutatedSubstitute1WithMinus13() {
        double[][] a = null;
        double[][] b = {{1.0}};
        assertFalse(DataUtilities.equal(a, b));
    }
    @Test
    public void testEqual_MutatedSubstitute1WithMinus14() {
        double[][] a = {{-1.0}};
        double[][] b = {{-1.0}};
        assertTrue(DataUtilities.equal(a, b));
    }
    // Mutated: equal to greater or equal
    @Test
    public void testEqual_MutatedEqualToGreaterOrEqual() {
        double[][] a = {{1.0}, {2.0}};
        double[][] b = {{1.0}};
        assertFalse(DataUtilities.equal(a, b)); // Mutated behavior: equal to greater or equal
    }
    //Mutated: Incremented (a++) double local variable number 2
    @Test
    public void testEqual_MutatedIncrementedALength() {
        double[][] a = {{1.0}, {2.0}};
        double[][] b = {{1.0}};
        assertFalse(DataUtilities.equal(a, b)); // Mutated behavior: Incremented (a++) double local variable number 2
    }
    // Mutated: Less than to not equal
    @Test
    public void testEqual_MutatedLessThanToNotEqual() {
        double[][] a = {{1.0}, {2.0}};
        double[][] b = {{1.0}};
        assertFalse(DataUtilities.equal(a, b)); // Mutated behavior: Less than to not equal
    }
    //Mutated: Substituted 0 with 1
    @Test
    public void testEqual_MutatedSubstitute0With1() {
        assertTrue(DataUtilities.equal(new double[0][0], new double[0][0])); // Mutated behavior: Substituted 0 with 1
    }
	// Mutated: Incremented (++a) double local variable number 2	
    @Test
    public void testEqual_MutatedIncrementedALengthAgain() {
        double[][] a = {{1.0}, {2.0}};
        double[][] b = {{1.0}};
        assertFalse(DataUtilities.equal(a, b)); // Mutated behavior: Incremented (++a) double local variable number 2
    }
    // Mutant 205_4: Incremented (a++) integer array field
    @Test
    public void testMutant205_4() {
        Mockery mockingContext = new Mockery();
        final Values2D values = mockingContext.mock(Values2D.class, "values2D");
    ...
    // Mutant 205_6: Decremented (a--) integer array field
    @Test
    public void testMutant205_6() {
        Mockery mockingContext = new Mockery();
        final Values2D values = mockingContext.mock(Values2D.class);
        mockingContext.checking(new Expectations() {{
            one(values).getColumnCount();
        ...
        int[] validRows = {0};
        double expectedTotal = 1.0; // Sum of all values in the column

        double actualTotal = DataUtilities.calculateRowTotal(values, 0, validRows);
...
//    }
	...
	@Test
	public void testCreateNumberArray2DMoreThan10ElementsMutant() {
	    double[][] data = new double[11][];
	    for (int i = 0; i < data.length; i++) {
	        data[i] = new double[]{i * 2};
	    }
	    Number[][] result = DataUtilities.createNumberArray2D(data);
	    assertEquals("Failed...", data.length, result.length);
	}


	@Test
	public void testMutantColumnTotalWithNonNullValues() {
	    // Prepare test data
	    DefaultKeyedValues2D testData = new DefaultKeyedValues2D();
	    testData.addValue(2.0, 0, 1);
	    testData.addValue(4.0, 1, 1);
	    testData.addValue(6.0, 2, 1);

	    // Define column and valid rows
	    int column = 1;
	    int[] validRows = {0, 1, 2};

	    // Calculate expected total
	    double expectedTotal = 4.0; // Sum of values in valid rows

	    // Calculate the actual total
	    double actualTotal = DataUtilities.calculateRowTotal(testData, column, validRows);

	    // Assert the result
	    assertEquals("Incorrect total for mutant column", expectedTotal, actualTotal, 0.000001);
	}


	@Test
	public void testCreateNumberArrayPositiveDataMutant() {
	...
	// cumulative percentages with null vals
	@Test
	public void testGetCumulativePercentagesNullValuesMutant() {
